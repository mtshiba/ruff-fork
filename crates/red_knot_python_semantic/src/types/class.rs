...
impl<'db> Class<'db> {
    ...
    fn implicit_instance_attribute(
        ...
        for (scope, symbol) in attribute_symbols(db, class_body_scope, name) {
            let use_def = use_def_map(db, scope.to_scope_id(db, class_body_scope.file(db)));
            for attribute_assignment in use_def.attribute_assignments(symbol) {
                    ...
                    DefinitionKind::WithItem(with_item) => {
                        match with_item.target_kind() {
                            TargetKind::Sequence(_, unpack) => {
                                ...
                                let unpacked = infer_unpack_types(db, unpack);
                                let name_ast_id =
                                    with_item.target().scoped_expression_id(db, scope);
                                let inferred_ty = unpacked.expression_type(name_ast_id);
                            ...
                            TargetKind::Name => {
                                ...
                                let context_ty =
                                    infer_expression_type(db, with_item.context_expr());
